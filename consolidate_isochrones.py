...
for mode in MODES.keys():    
    for tier in ISOCHRONE_TIERS:
        ...
        max_input_mtime = max([pathlib.Path(g['source_file'].values[0]).stat().st_mtime for g in gdf_isochrones[mode][tier]])
        # print(f"Files contributing to {mode} {tier}: {max_input_mtime=}")
        isochrone_concatenated_path = pathlib.Path(f"data/isochrones_concatenated/{mode}/{tier}.geojson")
        if isochrone_concatenated_path.exists():
            ...
            if existing_mtime >= max_input_mtime:
                print(f"SKIP: Output file {isochrone_concatenated_path} is newer than input files. Skipping concatenation.")
                continue

        gdf_isochrones_concatenated[mode][tier] = gpd.GeoDataFrame(pd.concat(gdf_isochrones[mode][tier], ignore_index=True))
        ...
        gdf_isochrones_concatenated[mode][tier] = gdf_isochrones_concatenated[mode][tier].dissolve(by=['type', 'minutes'], as_index=False)
        
        isochrone_concatenated_path = pathlib.Path(f"data/isochrones_concatenated/{mode}/{tier}.geojson")
        isochrone_concatenated_path.parent.mkdir(parents=True, exist_ok=True)
        gdf_isochrones_concatenated[mode][tier].to_file(isochrone_concatenated_path, driver="GeoJSON")