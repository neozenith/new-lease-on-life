...
class StateBoundaryFilter:
    ...
    def load_state_polygon(self) -> gpd.GeoDataFrame:
        ...
        if state_gdf.empty:
            raise ValueError(
                f"{self.state_name} polygon not found in state polygons file. Available states: {', '.join(available_states)}"
            )
    ...
    def process_boundary_file(self, input_file: Path) -> int | None:
        ...
        try:
            ...
            log.info(
                f"  Kept {filtered_count}/{original_count} features ({filtered_count/original_count*100:.1f}%)"
            )
...
""",
    ...
    parser.add_argument(
        "--state", default="Victoria", help="State name to filter by (default: Victoria)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--limit", type=int, metavar="N", help="Process only the first N files (for testing)"
    )
    parser.add_argument("--verbose", action="store_true", help="Enable detailed logging")

    args = parser.parse_args()
    # Create and run the filter
    filter = StateBoundaryFilter(state_name=args.state, dry_run=args.dry_run, verbose=args.verbose)
