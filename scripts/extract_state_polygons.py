...
def main():
    ...
    # Define paths
    input_file = Path(
        "data/originals_converted/boundaries/SA4_2021_AUST_SHP_GDA2020/SA4_2021_AUST_GDA2020.parquet"
    )
    ...
    log.info("States/Territories processed:")
    for idx, row in states_gdf.iterrows():
        area_info = (
            f", Area: {row['total_area_sqkm']:.2f} km²"
            if "total_area_sqkm" in states_gdf.columns
            else ""
        )
