...
HUE_FOR_MODE = {mode: (i / len(ALL_MODES) + float_hue_offset) % 1.0 for i, mode in enumerate(ALL_MODES)}
...
for m, mode in enumerate(MODES.keys()):
    ...
    for t, tier in enumerate(ISOCHRONE_TIERS):
        ...
        isochrone_colors[f"{mode}-{tier}"] = rgba_float_to_255(hsv_to_rgb(float_hue, float_saturation, 0.8, ISOCHRONE_OPACITY))

ptv_color_lookup = {m: rgba_float_to_255(hsv_to_rgb(HUE_FOR_MODE[m], 0.8, 0.8, PTV_OPACITY))  for m in PTV_MODES}
...
gdf_ptv_lines = gdf_ptv_lines[~gdf_ptv_lines["MODE"].isin(["METRO BUS", "REGIONAL BUS", "REGIONAL COACH", "SKYBUS", "INTERSTATE TRAIN"])]
...
gdf_ptv_stops = gdf_ptv_stops[gdf_ptv_stops['MODE'].isin(["REGIONAL TRAIN", "METRO TRAIN", "METRO TRAM"])]
...
for mode in MODES.keys():    
    for tier in ISOCHRONE_TIERS:
        ...
        isochrone_concatenated_path = pathlib.Path(f"data/isochrones_concatenated/{mode}/{tier}.geoparquet")
        gdf_isochrones_concatenated[mode][tier] = gpd.read_parquet(isochrone_concatenated_path) 

        if ((mode == "foot" and (tier in ["15", "5"])) 
...
deck_spec = pdk.Deck(
    ...
    api_keys= {
        "google_maps": os.environ.get("GOOGLE_MAPS_API_KEY", ""),  # Replace with your Google Maps API key
    }
)
