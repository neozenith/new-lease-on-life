...
def export_shapefile_to_geojson(
    ...
    try:
        ...
        logger.info(f"Reading shapefile: {shapefile_path} {shapefile_path.stat().st_size / 1024 / 1024:.2f}Mb")
        ...
        if filter_columns:
            ...
            if valid_columns:
                ...
                logger.warning(f"No requested columns found in dataset, keeping all columns")
...
def process_shapefiles(
    ...
    for shapefile in tqdm(shapefiles, desc="Exporting shapefiles"):
        try:
            geojson_path = export_shapefile_to_geojson(
                shapefile, 
                output_dir, 
                simplify_tolerance
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", default=DATA_DIR, type=Path, help="Data directory containing shapefiles")
    parser.add_argument("--output-dir", default=OUTPUT_DIR, type=Path, help="Output directory for GeoJSON files")
    parser.add_argument("--simplify", type=float, help="Tolerance for geometry simplification (in degrees)")
